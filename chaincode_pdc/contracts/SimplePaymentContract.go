...
func (spc *SimplePaymentContract) RegisterUser(ctx contractapi.TransactionContextInterface, bank string) (error) {
	...
	if !ok {
		return fmt.Errorf("User details not found in the transient map input")
	}
	...
	if len(accountUserInput.Address) == 0 {
		return fmt.Errorf("Address field must be a non-empty string")
	}
	
	if len(bank) == 0 {
		return fmt.Errorf("Bank field must be a non-empty string")
	}
	...
	if userAsBytes != nil {
		...
		return fmt.Errorf("This User already exists %v", id)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to marshall user info: %v", err)
	}
...
// RegisterUserAccount : User registers his account
func (spc *SimplePaymentContract) RegisterUserAccount(ctx contractapi.TransactionContextInterface, id string, bank string) (*Account, error) {
...
func (spc *SimplePaymentContract) ApproveKYCStatus(ctx contractapi.TransactionContextInterface, id string) (bool, error) {
	...
	requiredAffiliation,_ := calculateOperatorAffiliation(ctx);

	e := ctx.GetClientIdentity().AssertAttributeValue("hf.Affiliation", requiredAffiliation)
	if e != nil {
		return false, fmt.Errorf("User must be operator to provide kyc approval: %v", e)
	}
...
func (spc *SimplePaymentContract) Transfer(ctx contractapi.TransactionContextInterface, remitter string, beneficiary string, amount int64) (string, error) {
	...
	ctx.GetStub().SetEvent("TransferEvent",transactionBytes);
...
func calculateOperatorAffiliation(ctx contractapi.TransactionContextInterface) (string,error) {
	// Get the MSP ID of submitting client identity
	clientMSPID, err := ctx.GetClientIdentity().GetMSPID()
	if err != nil {
		return "", fmt.Errorf("failed to get verified MSPID: %v", err)
	...
	if(clientMSPID == "HDFCMSP"){
		affilation = "hdfc.operator"
	} else if (clientMSPID == "SBIMSP"){
		affilation = "sbi.operator"
	}

	return affilation, nil
}
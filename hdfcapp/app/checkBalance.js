...
async function checkBalance(userId, bankUserId) {
	let response;
	try {
		...
		try {
            ...
            console.log('\n**************** As HDFC Client ****************');
            console.log('\n--> Evaluate Transaction: Balance');
            result = await contract.evaluateTransaction('Balance', bankUserId);
            console.log(`<-- result: ${prettyJSONString(result.toString())}`);
			response = { success: true, message: `Balance for account ${bankUserId} is ${prettyJSONString(result.toString())}`};
		}catch(error){

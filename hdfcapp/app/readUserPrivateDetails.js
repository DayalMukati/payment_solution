...
async function readUserPrivateDetails(userId) {
	let response;
	try {
		...
		try {
            ...
            console.log('\n**************** As HDFC Client ****************');
            console.log('\n--> Evaluate Transaction: ReadUserFromCollection');
            result = await contract.evaluateTransaction('ReadUserFromCollection', 'HDFCMSPUserCollection', `${userId}@HDFC`);
...
//module.exports = readUserPrivateDetails;
readUserPrivateDetails('dayal');

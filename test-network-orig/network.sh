...
function createOrgs() {
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/hdfc/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating HDFC Identities"

    createHDFC

    infoln "Creating SBI Identities"
  ...
  infoln "Generating CCP files for HDFC and SBI"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/hdfc/msp organizations/fabric-ca/hdfc/tls-cert.pem organizations/fabric-ca/hdfc/ca-cert.pem organizations/fabric-ca/hdfc/IssuerPublicKey organizations/fabric-ca/hdfc/IssuerRevocationPublicKey organizations/fabric-ca/hdfc/fabric-ca-server.db'
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/sbi/msp organizations/fabric-ca/sbi/tls-cert.pem organizations/fabric-ca/sbi/ca-cert.pem organizations/fabric-ca/sbi/IssuerPublicKey organizations/fabric-ca/sbi/IssuerRevocationPublicKey organizations/fabric-ca/sbi/fabric-ca-server.db'
